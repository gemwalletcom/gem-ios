...
struct URLParserTests {
    ...
    func assetUrl() async throws {
        ...
        #expect(chainAction == .deepLink(.asset(AssetId(chain: .bitcoin, tokenId: .none))))
        ...
        #expect(tokenAction == .deepLink(.asset(AssetId(chain: .ethereum, tokenId: "0xdAC17F958D2ee523a2206206994597C13D831ec7"))))
    }
    ...
    func assetUrlWithLocalePrefix() async throws {
        ...
        #expect(tokenAction == .deepLink(.asset(AssetId(chain: .solana, tokenId: "JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN"))))

        let extendedLocale = try URLParser.from(url: URL(string: "https://gemwallet.com/zh-cn/tokens/solana")!)
        #expect(extendedLocale == .deepLink(.asset(AssetId(chain: .solana, tokenId: .none))))
    }
    ...
    func swapUrl() async throws {
        ...
        #expect(swapFromOnly == .deepLink(.swap(AssetId(chain: .ethereum, tokenId: nil), nil)))
        ...
        #expect(swapFromTo == .deepLink(.swap(
    ...
    func perpetualsUrl() async throws {
        ...
        #expect(perpetualsAction == .deepLink(.perpetuals))
    }
    ...
    func rewardsUrl() async throws {
        #expect(try URLParser.from(url: URL(string: "https://gemwallet.com/join?code=gemcoder")!) == .deepLink(.rewards(code: "gemcoder")))
        #expect(try URLParser.from(url: URL(string: "https://gemwallet.com/rewards?code=gemcoder")!) == .deepLink(.rewards(code: "gemcoder")))
    }

    @Test
    func gemSchemeRewardsUrl() async throws {
        #expect(try URLParser.from(url: URL(string: "gem://join?code=gemcoder")!) == .deepLink(.rewards(code: "gemcoder")))
        #expect(try URLParser.from(url: URL(string: "gem://rewards?code=gemcoder")!) == .deepLink(.rewards(code: "gemcoder")))
    }

    @Test
    func buyUrl() async throws {
        let buyChain = try URLParser.from(url: URL(string: "https://gemwallet.com/buy/bitcoin")!)
        #expect(buyChain == .deepLink(.buy(AssetId(chain: .bitcoin, tokenId: nil), amount: nil)))
        ...
        #expect(buyToken == .deepLink(.buy(AssetId(chain: .ethereum, tokenId: "0xdAC17F958D2ee523a2206206994597C13D831ec7"), amount: nil)))

        let buyWithAmount = try URLParser.from(url: URL(string: "https://gemwallet.com/buy/bitcoin?amount=100")!)
        #expect(buyWithAmount == .deepLink(.buy(AssetId(chain: .bitcoin, tokenId: nil), amount: 100)))
        ...
        #expect(buyTokenWithAmount == .deepLink(.buy(AssetId(chain: .ethereum, tokenId: "0xdAC17F958D2ee523a2206206994597C13D831ec7"), amount: 50)))
    }

    @Test
    func sellUrl() async throws {
        let sellChain = try URLParser.from(url: URL(string: "https://gemwallet.com/sell/bitcoin")!)
        #expect(sellChain == .deepLink(.sell(AssetId(chain: .bitcoin, tokenId: nil), amount: nil)))

        let sellWithAmount = try URLParser.from(url: URL(string: "https://gemwallet.com/sell/ethereum?amount=200")!)
        #expect(sellWithAmount == .deepLink(.sell(AssetId(chain: .ethereum, tokenId: nil), amount: 200)))
    }
    ...
    func setPriceAlertUrl() async throws {
        ...
        #expect(alertWithPrice == .deepLink(.setPriceAlert(AssetId(chain: .bitcoin, tokenId: nil), price: 50000)))
        ...
        #expect(alertTokenWithPrice == .deepLink(.setPriceAlert(AssetId(chain: .ethereum, tokenId: "0xdAC17F958D2ee523a2206206994597C13D831ec7"), price: 1.5)))
    }
    ...
    func setPriceAlertUrlWithoutPrice() async throws {
        ...
        #expect(alertWithoutPrice == .deepLink(.setPriceAlert(AssetId(chain: .bitcoin, tokenId: nil), price: nil)))
    }
}
