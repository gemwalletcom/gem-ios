    ...
    public func importWallet(
        ...
        return try await queue.asyncTask { [walletKeyStore] in
            ...
            case .phrase(let words, let chains):
                try walletKeyStore.importWallet(name: name, words: words, chains: chains, password: password, source: source)
            case .single(let words, let chain):
                try walletKeyStore.importWallet(name: name, words: words, chains: [chain], password: password, source: source)
