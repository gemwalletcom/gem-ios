...
public extension GemTransactionInputType {
    func getAsset() -> GemAsset {
        ...
        case .earn(asset: let asset, earnType: _, earnData: _): asset
        }
    }
}
...
public extension GemTransactionInputType {
    func map() throws -> TransferDataType {
        ...
        case .earn(let asset, let earnType, let earnData):
            return try TransferDataType.earn(asset.map(), earnType.map(), earnData.map())
        }
    }
}
...
public extension TransferDataType {
    func map() throws -> GemTransactionInputType {
        ...
        case .earn(let asset, let earnType, let earnData):
            return .earn(asset: asset.map(), earnType: earnType.map(), earnData: earnData.map())
        }
    }
}
