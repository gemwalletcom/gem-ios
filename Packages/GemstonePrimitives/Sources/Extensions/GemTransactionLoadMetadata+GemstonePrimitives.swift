...
extension GemTransactionLoadMetadata {
    public func map() throws -> TransactionLoadMetadata {
        ...
        case .evm(let nonce, let chainId, let stakeData, let yieldData):
            return .evm(nonce: UInt64(nonce), chainId: UInt64(chainId), stakeData: stakeData?.map(), yieldData: yieldData?.map())
